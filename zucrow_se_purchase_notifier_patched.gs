...
function onOpen() {
  SpreadsheetApp.getUi()
    .createMenu('📧 Order Notifier (GitHub Dual Logo v2.0)')
...
function viewRepositoryInfo() {
  try {
    var repoInfo = getRepositoryInfo();
    var message = 'GITHUB DUAL LOGO REPOSITORY INFO\n\n';
...
function testEmailForRow() {
  try {
    ...
    var ui = SpreadsheetApp.getUi();
    var repoInfo = getRepositoryInfo();
    var message = 'GITHUB DUAL LOGO TEST EMAIL:\n\n';
    ...
    message += 'Repository: ' + repoInfo.url + '\n';
    message += 'Version: ' + repoInfo.version + '\n\n';
    message += 'Send REAL email with dual logo support?';
    
    var response = ui.alert('Confirm Dual Logo Test', message, ui.ButtonSet.YES_NO);
    
    if (response !== ui.Button.YES) {
      return;
    }
    
    logToSheet('MANUAL DUAL LOGO TEST: Processing row ' + row);
    
    try {
      processOrderRow(sheet, row, mapping);
      logToSheet('DUAL LOGO TEST EMAIL: Sent to ' + rowData.email + ' with logos: ' + logoPreview);
      
      ui.alert('Test Email Sent!', 
        'GitHub dual logo test email sent successfully! 🎉\n\n' +
      ...
      if (error.toString().indexOf('Duplicate email prevented') !== -1) {
        ...
        if (overrideResponse === ui.Button.YES) {
          ...
          ui.alert('Test Email Sent!', 
            'GitHub dual logo test email sent (duplicate override)! 🎉\n\n' +
...
function testConfiguration() {
  try {
    logToSheet('=== DUAL LOGO CONFIGURATION TEST ===');
    ...
    logToSheet('=== DUAL LOGO CONFIGURATION TEST COMPLETE ===');
...
function initializeSystem() {
  try {
    logToSheet('=== GITHUB DUAL LOGO SYSTEM INITIALIZATION ===');
    ...
    logToSheet('=== GITHUB DUAL LOGO SYSTEM INITIALIZATION COMPLETED ===');
    
    var repoInfo = getRepositoryInfo();
    var message = 'GitHub Dual Logo System Initialized!\n\n';
    message += 'Configuration validated\n';
    message += 'Data validation rules applied\n';
    message += 'Helper columns configured\n';
    message += 'Dual logo integration tested\n\n';
    ...
    SpreadsheetApp.getUi().alert('GitHub Dual Logo System Initialized', message, SpreadsheetApp.getUi().ButtonSet.OK);
    
  } catch (error) {
    logToSheet('GITHUB DUAL LOGO INIT ERROR: ' + error.toString());
    SpreadsheetApp.getUi().alert('Initialization Failed', 
      'GitHub dual logo system initialization failed: ' + error.toString(), 
      SpreadsheetApp.getUi().ButtonSet.OK);
  }
}
...
function validateConfiguration() {
  ...
  try {
    ...
    if (issues.length > 0) {
      ...
      logToSheet('Dual logo configuration validation passed');
      return true;
    }
  } catch (error) {
...
function checkSetup() {
  try {
    ...
    SpreadsheetApp.getUi().alert('GitHub Dual Logo Setup Check', message, SpreadsheetApp.getUi().ButtonSet.OK);
...
function configureSenderEmail() {
  try {
    ...
    var message = 'EMAIL SENDER CONFIGURATION (GitHub Dual Logo Edition)\n\n';
...
function viewCurrentConfig() {
  try {
    var sheet = getMainSheet();
    var repoInfo = getRepositoryInfo();
    var message = 'CURRENT SYSTEM CONFIGURATION (GitHub Dual Logo Edition)\n\n';
...
function viewEmailBackupLog() {
  try {
    ...
    message += 'The log shows all email attempts with GitHub dual logo version tracking.\n\n';
...
function resetRateLimits() {
  try {
    ...
    if (rateLimitSheet) {
      ...
      logToSheet('ADMIN ACTION: Rate limits reset by user (GitHub Dual Logo Edition)');
    }
...
function auditDataIntegrity() {
  try {
    ...
    logToSheet('=== GITHUB DUAL LOGO DATA INTEGRITY AUDIT STARTED ===');
    ...
    logToSheet('=== GITHUB DUAL LOGO DATA INTEGRITY AUDIT COMPLETED ===');
    ...
    message += '\nDual logo system operating normally.';
...
// =============================================================================
// END OF GITHUB DUAL LOGO PURCHASE ORDER EMAIL NOTIFICATION SYSTEM
// =============================================================================
